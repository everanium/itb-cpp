 ...
 * (`-litb3_cpp -litb3` with an embedded RPATH) — no runtime symbol
 ...
 *
 * Errors. Every fallible entry throws itb::Error on a non-OK libitb3
...
/* Thrown by every fallible entry on a non-OK libitb3 status. what()
 ...
 * decisions stay inside libitb3. Move-only; the destructor cancels
...
public:
     ...
     * supply both (a half-supplied pair is rejected by libitb3). The
