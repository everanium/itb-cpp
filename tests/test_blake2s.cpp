// test_blake2s.cpp — BLAKE2s low-level cipher coverage.
//
// Mirrors bindings/c/tests/test_blake2s.c on the C++ free-function
// cipher surface. BLAKE2s ships at a single native width (256).
...
constexpr int kBlake2sWidth      = 256;
...
TEST_CASE("blake2s alternate key_bits round-trip", "[blake2s][altkb]") {
...
TEST_CASE("blake2s auth round-trip across MACs", "[blake2s][auth]") {
...
TEST_CASE("blake2s cross-seed decrypt does not recover plaintext",
          "[blake2s][cross]") {
...
TEST_CASE("blake2s seed invariants", "[blake2s][invariants]") {
    itb::Seed n{kPrimitive, kDefaultKeyBits};
    REQUIRE(n.width() == kBlake2sWidth);
