// test_blake3.cpp — BLAKE3 low-level cipher coverage.
//
// Mirrors bindings/c/tests/test_blake3.c on the C++ free-function
// cipher surface. BLAKE3 ships at a single native width (256).
...
constexpr const char* kPrimitive = "blake3";
constexpr int kBlake3Width       = 256;
...
TEST_CASE("blake3 single ouroboros round-trip", "[blake3][single]") {
...
TEST_CASE("blake3 triple ouroboros round-trip", "[blake3][triple]") {
    for (std::size_t sz : kSizes) {
        SECTION(std::string{"size="} + std::to_string(sz)) {
            auto plaintext = token_bytes(sz);
            itb::Seed n {kPrimitive, kDefaultKeyBits};
...
TEST_CASE("blake3 alternate key_bits round-trip", "[blake3][altkb]") {
...
TEST_CASE("blake3 auth round-trip across MACs", "[blake3][auth]") {
...
TEST_CASE("blake3 cross-seed decrypt does not recover plaintext",
          "[blake3][cross]") {
...
TEST_CASE("blake3 seed invariants", "[blake3][invariants]") {
    itb::Seed n{kPrimitive, kDefaultKeyBits};
    REQUIRE(n.width() == kBlake3Width);
