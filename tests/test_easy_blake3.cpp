// test_blake3.cpp — BLAKE3-focused Encryptor (Easy Mode) coverage.
//
// Mirrors bindings/c/tests/test_easy_blake3.c — Single + Triple
// Ouroboros round-trips (encrypt / decrypt + encrypt_auth /
// decrypt_auth) across the size grid 32 B / 4 KiB / 64 KiB.
// BLAKE3 ships at a single width (256-bit); the alternate
...
std::vector<std::uint8_t> token_bytes(std::size_t len) {
    static std::uint64_t ctr = 0xB1A3D00CDADAFEEDULL;
...
TEST_CASE("blake3 single ouroboros auth round-trip",
          "[blake3][single][auth]") {
    for (std::size_t sz : kSizes) {
        SECTION(std::string{"size="} + std::to_string(sz)) {
            auto plaintext = token_bytes(sz);
...
TEST_CASE("blake3 triple ouroboros auth round-trip",
          "[blake3][triple][auth]") {
    for (std::size_t sz : kSizes) {
        SECTION(std::string{"size="} + std::to_string(sz)) {
            auto plaintext = token_bytes(sz);
...
TEST_CASE("blake3 alternate key_bits 2048", "[blake3][altkb]") {
...
TEST_CASE("blake3 tamper resistance via export-import", "[blake3][tamper]") {
