...
`itb::wrapper::ffi_name(cipher)` returns the canonical short name (`"aescmac"` / `"chacha20"` / `"siphash24"`) as a non-owning view over the process-lifetime interned C string the underlying libitb owns.
...
```sh
make eitb
./eitb/build/eitb              # 24 PASS, 0 FAIL
