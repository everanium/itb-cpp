...
`itb::wrapper::ffi_name(cipher)` returns the canonical short name (`"aes"` / `"chacha"` / `"siphash"`) as a non-owning view over the process-lifetime interned C string the underlying libitb owns.
