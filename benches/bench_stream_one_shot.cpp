 ...
 * path (a single FFI round trip through the Pipeline's stream
 * chain). */
    ...
    try {
        ...
        for (std::size_t size : sizes) {
            ...
            bench_case("stream_one_shot", size, [&] {
                (void)pipe.encrypt_stream_one_shot_into(itb::as_bytes(plain),
            ...
            std::vector<std::uint8_t> dec_wire = pipe.encrypt_stream_one_shot(
                itb::as_bytes(plain));
            std::vector<std::uint8_t> dec_out(size);
            bench_case("stream_one_shot-dec", size, [&] {
                (void)pipe.decrypt_stream_one_shot_into(
        ...
        std::fprintf(stderr, "bench_stream_one_shot: %s\n", e.what());
        return 1;
    }
}
